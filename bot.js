...
let spread = JSON.parse(fs.readFileSync('./antispamlinks.json' , 'utf8'));
...
message.channel.send(`successfully set the antisoamlinks to \`[off]\``)
          fs.writeFile("./antispamlinks.json", JSON.stringify(spread), (err) => {
...
message.channel.send(`successfully set the antisoamlinks to \`[on]\``)
          fs.writeFile("./antispamlinks.json", JSON.stringify(spread), (err) => {
            if (err) console.error(err)
            .catch(err => {
              console.error(err);
    ...
    if(message.content.includes('http://www.gmail.com/')){
        ...
        message.delete()
    return message.reply(`\`🚧 The server is protected from spam links here. Sorry, you can not post this link 🚧\``)
    }
});
...
client.on('message', message => {
    ...
    if(message.content.includes('https://www.instagram.com/')){
        if(!message.member.hasPermission('MANAGE_ROLES' , 'ADMINSTRATOR', 'MANAGE_GUILD', 'MANAGE_ROLES', 'MANAGE_CHANNELS')) return;
            if(!spread[message.guild.id]) spread[message.guild.id] = {
        onoff: 'Off'
            }
        if(spread[message.guild.id].onoff === 'Off') return;
