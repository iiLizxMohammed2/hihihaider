...
exports.run = (client, message, args, tools) => {
  ...
  if (!args[0]) return message.channel.send(`\`${message.author.username}\`, pelase insert the [URL] \n Example: !short [The URL]`)
  ...
  if (!args[1]) { // If the second argument in the message is undefined, run this
    ...
    shorten.shorten(args[0], function(res) { // This will run the shorten function and pass it 'res'
      if (res.startsWith('Error:')) return message.channel.send(`\`${message.author.username}\`, please insert a valid [URL]`); // The only possible error here would be that it's not a valid URL.
    ...
    shorten.custom(args[0], args[1], function(res) { // This is sort of the same thing from earlier, although this will pass the first and second arguments to the shortener, then return 'res'
      ...
      message.channel.send(` The URL is ready -> **<${res}>**`); // If no errors encountered, it will return the link.
